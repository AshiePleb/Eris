...
fs.readdir("modules", (err, files) => {
  ...
  if (files.includes("BotManager.js") == false) {
    console.log("[Modules] Module BotManager was not found. This can limit your module ServerSettings abilities, which is the opposite of what the bot was made for.")
  }
})
...
bot.on("ready", () => {
  console.log("[Shards] All shards are ready.");
  bot.editStatus(settings.get("presence"));
  ...
  for (var a in music) {
    if (!music.hasOwnProperty(a)) continue;
    if (music[a].queue.length > 0) {
      bot.createMessage(music[a].channel.id, {
        embed: {
          title: "Lost connection to Discord",
          description: "It seems like I lost connection to Discord while music was playing.\nI'll now try to resume the session (the current playing track will be restarted).\n\nSorry! If this issue persists, check [Discord's status page](https://status.discordapp.com) or join my [Support Server](https://discord.gg/JfCqSQS)."
        }
      })
      bot.leaveVoiceChannel(music[a].voice.id);
      voice = bot.guilds.get(music[a].guild.id).channels.get(music[a].voice.id);
      channel = bot.guilds.get(music[a].guild.id).channels.get(music[a].channel.id);
      guild = bot.guilds.get(music[a].guild.id);
      data.set("music." + a + ".voice", voice);
      data.set("music." + a + ".channel", channel);
      data.set("music." + a + ".guild", guild);
      setTimeout(function() {require("./modules/music.js").play(music[a].guild, music[a].queue[0].track);}, 1000);
    }
    else {
      data.del("music." + a);
    }
